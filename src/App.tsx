...
export default function App() {
  ...
  // Thesis Requirement: Blue-Green Cutover State (FR-05)
  ...
  const [telemetry, setTelemetry] = useState({
    ...
    throughput: 0,
    upload: 0,
    latency: 2, // NFR-01: Latency monitoring
    integrityScore: 100, // FR-04: Merkle Tree Match %
  });
  ...
  const handleConnectClick = () => {
    if (isConnected) {
      ...
      setWorkspaceTabs((prev) =>
        prev.filter((t) => t !== "Monitor: PG -> Mongo -> AWS"),
      );
  ...
  const handleCutover = () => {
    ...
    addLog(
      newState === "GREEN_MONGO"
        ? "CUTOVER INITIATED: Traffic switched to Target (Green)."
        : "ROLLBACK INITIATED: Traffic reverted to Source (Blue).",
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-slate-200 overflow-hidden font-sans text-slate-900 selection:bg-blue-200 relative">
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* A. Object Explorer - Conditional Render */}
        {showLeftPanel && (
        ...
        <div className="flex-1 flex flex-col bg-slate-50 min-w-0">
          <div className="flex bg-slate-200 border-b border-slate-300 h-6 items-end px-1 gap-1 shrink-0">
            {workspaceTabs.map((tab) => (
              ...
              >
                <span>{tab}</span>
                {tab !== "Start Page" && (
                  <X size={10} className="hover:bg-slate-300 rounded" />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="flex-1 flex overflow-hidden">
            ...
            {/* C. Right Telemetry (Only when connected AND enabled in View) */}
            {isConnected &&
              activeWorkspaceTab !== "Start Page" &&
              showRightPanel && (
                <div className="w-[220px] bg-slate-50 border-l border-slate-300 flex flex-col shrink-0 z-10 shadow-[-2px_0_5px_rgba(0,0,0,0.02)]">
                  ...
                  {/* FIX: Changed overflow-y-auto to overflow-hidden (Visually & Functionally Disabled Scroll) */}
                  <div className="p-3 overflow-hidden flex-1">
                    ...
                    <div className="bg-white border border-slate-200 p-3 rounded mb-4 shadow-sm">
                      <h4 className="text-[11px] font-bold text-slate-800 mb-3 flex items-center gap-2">
                        <Server size={12} className="text-blue-500" /> Node
                        Metrics
                      </h4>
                      <TelemetryBar
                        label="CPU Usage"
                    ...
                    <div className="bg-white border border-slate-200 p-3 rounded mb-4 shadow-sm">
                      <h4 className="text-[11px] font-bold text-slate-800 mb-3 flex items-center gap-2">
                        <Network size={12} className="text-green-500" />{" "}
                        Throughput
                      </h4>
                    ...
                    {isAwsEnabled && (
                      <div className="bg-white border border-slate-200 p-3 rounded mb-4 shadow-sm">
                        <h4 className="text-[11px] font-bold text-slate-800 mb-3 flex items-center gap-2">
                          <Cloud size={12} className="text-orange-500" /> Cloud
                          Sync
                        </h4>
                        <TelemetryBar
                          label="S3 Upload"
                          value={telemetry.upload}
                          color="bg-orange-500"
                        />

...
export function BlueGreenSimulationView() {
  ...
  const runQuery = () => {
    showToast("Data retrieved seamlessly via Zero-Downtime Routing.");
  };
  ...
  return <div className="h-full overflow-auto bg-slate-100 p-4 text-xs text-slate-700 relative">
    {toastMessage && (
      <div className="fixed right-5 top-16 z-50 rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700 shadow">
        {toastMessage}
      </div>
    )}

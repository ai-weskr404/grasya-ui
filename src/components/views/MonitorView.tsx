    ...
    <div className="flex flex-col h-full">
        ...
        <div className="bg-slate-50 border border-slate-200 p-6 rounded-sm flex items-center justify-between relative overflow-hidden gap-2">
          ...
          <div className="relative flex flex-col items-center gap-2 z-10 w-24">
            <div className={`
              ...
              `}>
                <DatabaseIcon className={trafficState === 'BLUE_POSTGRES' ? "text-blue-600" : "text-slate-400"} size={20} />
            </div>
          ...
          <div className="flex flex-col items-center gap-2 z-10 w-24">
            <div className="w-12 h-12 bg-white border border-slate-300 shadow-sm rounded flex items-center justify-center">
                <Icon icon={iconMap.Server} className="text-purple-600" size={20} />
            </div>
            <div className="text-center">
          ...
          <div className="relative flex flex-col items-center gap-2 z-10 w-24">
            <div className={`
                ...
                `}>
                <DatabaseIcon className={trafficState === 'GREEN_MONGO' ? "text-green-600" : "text-slate-400"} size={20} />
                <div className={`
          ...
          <div className="flex items-center gap-1 flex-1 justify-center z-10">
            <div className={`h-1 flex-1 bg-slate-300 rounded relative overflow-hidden`}>
                {isRunning && isAtlasEnabled && <div className="absolute inset-0 bg-orange-400 animate-progress-indeterminate" style={{ animationDelay: '0.5s' }} />}
            </div>
            {isAtlasEnabled && <Icon icon={iconMap.UploadCloud} size={14} className="text-slate-400" />}
            <div className={`h-1 flex-1 bg-slate-300 rounded relative overflow-hidden`}>
                {isRunning && isAtlasEnabled && <div className="absolute inset-0 bg-orange-400 animate-progress-indeterminate" style={{ animationDelay: '0.7s' }} />}
            </div>
          </div>

          {/* 3. MongoDB Atlas Cloud */}
          <div className={`flex flex-col items-center gap-2 z-10 w-24 ${!isAtlasEnabled ? 'opacity-30 grayscale' : ''}`}>
            <div className="w-12 h-12 bg-white border border-slate-300 shadow-sm rounded flex items-center justify-center">
                <Icon icon={iconMap.Cloud} className="text-orange-500" size={20} />
            </div>
            <div className="text-center">
                <div className="text-[10px] font-bold text-slate-700">MongoDB Storage</div>
      ...
      <div className="flex-1 flex flex-col min-h-0">
          <div className="flex bg-slate-100 border-b border-slate-300 h-7 items-center px-2 gap-4 shrink-0">
            ...
            <button className="text-[11px] font-medium text-slate-500 hover:text-slate-800 h-full px-1">Atlas Events</button>
        </div>
